...
$(document).ready(function () {
    ...
    $('#showstart').click( function() {
        $('#modal').hide();
        $('#replay').hide();
        //paint random individual rockets
        timeoutID = setTimeout(paintRandom, 1000);
    });

    function paintRandom() {
        RandomRocket();
        paintScreen();
        timeoutID = setTimeout(paintRandom, 2000);
        if (timeoutID > 8){
            //start the finale
            for (var i = 0; i < 10; i++) {
                RandomRocket();
    ...
    function RandomRocket() {
        var new_rocket = new Rocket (randomInt(0,w),  randomInt(50,500));
